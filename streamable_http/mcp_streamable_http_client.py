...
Usage: python streamable_http/mcp_streamable_http_client.py "Compute 7 * 6, then add 5."
"""
    ...
    def _wait_server_ready(self, timeout=5):
        ...
        while time.time() < deadline:
            try:
                ...
                time.sleep(0.1)
        raise RuntimeError("Streamable HTTP server not ready")

    def send(self, method, params={}):
        self._id += 1
        body = json.dumps({"jsonrpc": "2.0", "id": self._id, "method": method, "params": params}, ensure_ascii=False).encode("utf-8")
        req = request.Request(self.url, data=body, headers={"Content-Type": "application/json", "Accept": "application/json"}, method="POST")
...
def run_agent(user_message, max_iterations=5):
    mcp = MCPClient([sys.executable, os.path.join(os.path.dirname(__file__), "mcp_streamable_http_server.py")], "http://127.0.0.1:8766/mcp")
    mcp.initialize()
    openai_tools = [
...
if __name__ == "__main__":
    task = " ".join(sys.argv[1:]) if len(sys.argv) > 1 else "Compute 7 * 6, then add 5."
